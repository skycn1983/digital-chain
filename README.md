...
```bash
cd examples/transfer-dapp
npx http-server -p 8080 -o
```

[查看文档](examples/transfer-dapp/README.md)
...
[查看文档](examples/voting-dapp/README.md)
...
预期输出示例：
```
Node1: 3 total peers, 2 P2P peers  # 作为种子节点
Node2: 1 total peers, 1 P2P peers  # 连接到 Node1
Node3: 1 total peers, 1 P2P peers  # 连接到 Node1
```

### 交易广播测试

验证交易从节点1发送后，是否能被节点2和3收到：

```bash
node test-p2p-broadcast.js
```
...
```bash
# 基础测试：100笔交易，10并发
CONCURRENT=10 TOTAL_TX=100 node benchmark/performance.js

# 压力测试：1000笔交易，50并发
CONCURRENT=50 TOTAL_TX=1000 node benchmark/performance.js
```

**输出指标**:
- 总耗时、TPS（每秒交易数）
- 延迟统计：Min、Avg、Median、p95、p99、Max
- 最终链状态（区块数、pending数）
